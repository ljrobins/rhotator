...
def sn_cn_dn_am(x, k):
    ...
    K = (
        PI / (2 * agm(1.0, kp))
    )  # TODO: fix with this https://dlmf.nist.gov/19.8#i.p1 to fufill https://dlmf.nist.gov/22.20
...
def rf(x, y, z):
    for _ in range(4):
        lam = (
            ti.sqrt(x) * ti.sqrt(y) + ti.sqrt(y) * ti.sqrt(z) + ti.sqrt(z) * ti.sqrt(x)
        )
        x = (x + lam) / 4
        y = (y + lam) / 4
        z = (z + lam) / 4
...
def rj(x, y, z, p):
    ...
    while True:
        ...
        pndev = (mu - pn) / mu
        epslon = ti.max(
            ti.abs(xndev), ti.max(ti.abs(yndev), ti.max(ti.abs(zndev), ti.abs(pndev)))
        )
...
@ti.pyfunc
def elliptic_pi_incomplete(n: float, phi: float, ksquared: float) -> float:
